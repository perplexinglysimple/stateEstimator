...
static int use_wmm = 1;

int main(int argc, char** argv)
{
    const char* outPath = "ekf_boat_compare.csv";
    if (argc > 1 && argv[1] != NULL)
    {
        outPath = argv[1];
    }
    if (argc > 2 && argv[2] != NULL && strcmp(argv[2], "--no-wmm") == 0)
    {
        use_wmm = 0;
    }
    ...
    {
        LOG_ERROR("EKFInit() failed in boat export test.");
        return -1;
    }
    ...
    fprintf(f, "step");
    for (int i = 0; i < BOAT_STATE_DIM; ++i)
    {
        fprintf(f, ",x%d", i);
    }
    for (int i = 0; i < BOAT_STATE_DIM; ++i)
    ...
    for (int step = 0; step < 50; ++step)
    {
        ...
        {
            LOG_ERROR("EKFPredict() failed in boat export test.");
            fclose(f);
            return -1;
        }
        ...
        {
            LOG_ERROR("EKFUpdate() failed in boat export test.");
            fclose(f);
            return -1;
        }
        ...
        for (int i = 0; i < BOAT_STATE_DIM; ++i)
        {
            fprintf(f, ",%.10f", ACCESS_MATRIX(*(ekf.x), i, 0));
        }
        for (int i = 0; i < BOAT_STATE_DIM; ++i)
        {
            fprintf(f, ",%.10f", ACCESS_MATRIX(*(ekf.P), i, i));
        }
        fprintf(f, "\n");
    }
...
static void BoatMeasurement(EKFMatrix* x, EKFMatrix* z, EKFState* ekf, void* userData)
{
    ...
    if (ecefToGeodetic(px, py, pz, &lat_rad, &lon_rad, &alt_m) == 0)
    {
        ...
        {
            // Fixed NED field when WMM disabled
            Bn = 0.2;
            Be = 0.0;
            Bd = 0.45;
            double Bx_e, By_e, Bz_e;
