 ...
 *     that the system and measurement models are correctly formulated and represent the dynamics of the system
 * accurately. A poorly defined or inaccurate model can lead to numerical instability and singularities.
 ...
 *     uncertainty or variability in the system dynamics. If the algorithm is consistently encountering singularities,
 * you may need to adjust the values in the process noise covariance matrix to ensure a well-conditioned system.
 *  5. Consider alternative algorithms: If the above steps do not resolve the issue, you may need to explore alternative
 *     estimation algorithms. The EKF is just one approach, and there are other variants like the Unscented Kalman
 * Filter (UKF) or Particle Filters that can be more robust in handling non-linear or singular scenarios. I may
